...
CompileCtx *compile_ctx_new(TokenStream *ts) {
    ...
    ctx->const_pool = (const char **)malloc((size_t)ctx->const_pool_cap *
                                            sizeof(const char *));
    ctx->err = NULL;
    ctx->symbol = 0;
    return ctx;
...
int add_str_to_stack(CompileCtx *ctx, char *str) {
    ...
    if (idx < 0) {
        ...
        if (idx >= ctx->const_pool_cap) {
            ctx->const_pool_cap += 10;
            ctx->const_pool = (const char **)realloc(
                ctx->const_pool,
                (size_t)ctx->const_pool_cap * sizeof(const char *));
        }
    }
...
uint16_t decode_const_pool(char **const_strs, uint8_t *buffer) {
    uint16_t offset;
...
Val compile(TokenStream *ts, const char *opcode_path, char **err_out,
            int *symbol_out) {
...
{ if (err_out) *err_out = NULL; TokenStream ts; if (!tokenize_into(&ts, line,
strlen(line))) { if (err_out) *err_out = "tokenize failed"; return false;
    }
