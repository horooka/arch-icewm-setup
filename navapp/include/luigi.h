// Is <https://github.com/nakst/luigi/luigi2 (beta).h> with color support
...
#define UI_ASSERT(x)                                                           \
    do {                                                                       \
        if (!(x)) {                                                            \
            ...
            MessageBox(0,                                                      \
                       "Assertion failure on line " _UI_TO_STRING_2(__LINE__), \
                       0, 0);                                                  \
            ExitProcess(1);                                                    \
        }                                                                      \
    } while (0)
...
#define UI_MDI_CHILD_CALCULATE_LAYOUT(bounds, scale)                           \
    ...
    UIRectangle title =                                                        \
        UIRectangleAdd(bounds, UI_RECT_4(borderSize, -borderSize, 0, 0));      \
    title.b = title.t + titleSize;                                             \
    UIRectangle content = UIRectangleAdd(                                      \
        bounds, UI_RECT_4(borderSize, -borderSize, titleSize, -borderSize));
...
typedef enum UIMessage {
    ...
    UI_MSG_GET_CHILD_STABILITY, // dp = child element; return stable axes, 1
                                // (width) | 2 (height)
    ...
    UI_MSG_PRESSED_DESCENDENT, // dp = pointer to child that is/contains pressed
                               // element
    UI_MSG_INPUT_EVENTS_END,

    // Specific elements.
    UI_MSG_VALUE_CHANGED, // sent to notify that the element's value has changed
    UI_MSG_TABLE_GET_ITEM, // dp = pointer to UITableGetItem; return string
                           // length
    ...
    UI_MSG_TAB_SELECTED, // sent to the tab that was selected (not the tab pane
                         // itself)

    // Windows.
    UI_MSG_WINDOW_DROP_FILES, // di = count, dp = char ** of paths
    UI_MSG_WINDOW_ACTIVATE,
    UI_MSG_WINDOW_CLOSE, // return 1 to prevent default (process exit for
                         // UIWindow; close for UIMDIChild)
...
#define UI_RECT_4PD(x, y, w, h)                                                \
    ((UIRectangle){(x), ((x) + (w)), (y), ((y) + (h))})
...
#define UI_DRAW_CONTROL_STATE_FROM_ELEMENT(x)                                  \
    ((((x)->flags & UI_ELEMENT_DISABLED) ? UI_DRAW_CONTROL_STATE_DISABLED      \
                                         : 0) |                                \
...
#define UI_ELEMENT_DESTROY_DESCENDENT (1 << 31)
        ...
        int (*messageClass)(struct UIElement *element, UIMessage message,
                            int di /* data integer */,
                            void *dp /* data pointer */);
        int (*messageUser)(struct UIElement *element, UIMessage message, int di,
                           void *dp);
...
#define UI_WINDOW_MAXIMIZE (1 << 3)
        ...
        // Useful for tracking whether changes to the loaded document have been
        // saved.
...
typedef struct UILabel {
        ...
        char *label;
        ptrdiff_t labelBytes;
        uint32_t colorText;       // 0 = ui.theme.text
        uint32_t colorBackground; // 0 = no fill
} UILabel;
...
#define UI_SPLIT_PANE_VERTICAL (1 << 0)
        ...
        float weight;
        UIRectangle
            margin; // inset around left/splitter/right (like UIPanel.border)
} UISplitPane;
...
UIElement *UIElementCreate(size_t bytes, UIElement *parent, uint32_t flags,
                           int (*messageClass)(UIElement *, UIMessage, int,
                                               void *),
                           const char *cClassName);

UICheckbox *UICheckboxCreate(UIElement *parent, uint32_t flags,
...
UIMDIChild *UIMDIChildCreate(UIElement *parent, uint32_t flags,
                             UIRectangle initialBounds, const char *title,
                             ptrdiff_t titleBytes);
UIPanel *UIPanelCreate(UIElement *parent, uint32_t flags);
UIScrollBar *UIScrollBarCreate(UIElement *parent, uint32_t flags);
UISpacer *UISpacerCreate(UIElement *parent, uint32_t flags, int width,
                         int height);
UISplitPane *UISplitPaneCreate(UIElement *parent, uint32_t flags, float weight);
UITabPane *
UITabPaneCreate(UIElement *parent, uint32_t flags,
...
UIButton *UIButtonCreate(UIElement *parent, uint32_t flags, const char *label,
                         ptrdiff_t labelBytes);
void UIButtonSetLabel(UIButton *button, const char *string,
                      ptrdiff_t stringBytes);

UILabel *UILabelCreate(UIElement *parent, uint32_t flags, const char *label,
                       ptrdiff_t labelBytes);
void UILabelSetContent(UILabel *code, const char *content, ptrdiff_t byteCount);

UIImageDisplay *UIImageDisplayCreate(UIElement *parent, uint32_t flags,
                                     uint32_t *bits, size_t width,
                                     size_t height, size_t stride);
void UIImageDisplaySetContent(UIImageDisplay *display, uint32_t *bits,
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle,
                         int width, int height);
void UIWindowRegisterShortcut(UIWindow *window, UIShortcut shortcut);
void UIWindowPostMessage(UIWindow *window, UIMessage message,
                         void *dp); // Thread-safe.
void UIWindowPack(
    UIWindow *window,
    int width); // Change the size of the window to best match its contents.
...
void UIMenuAddItem(UIMenu *menu, uint32_t flags, const char *label,
                   ptrdiff_t labelBytes, void (*invoke)(void *cp), void *cp);
...
UITable *
UITableCreate(UIElement *parent, uint32_t flags,
              const char *columns /* separate with \t, terminate with \0 */);
int UITableHitTest(UITable *table, int x,
                   int y); // Returns item index. Returns -1 if not on an item.
int UITableHeaderHitTest(UITable *table, int x,
                         int y); // Returns column index or -1.
bool UITableEnsureVisible(
    UITable *table,
    int index); // Returns false if the item was already visible.
...
int UICodeHitTest(UICode *code, int x,
                  int y); // Returns line number; negates if in margin. Returns
                          // 0 if not on a line.
void UICodeInsertContent(UICode *code, const char *content, ptrdiff_t byteCount,
                         bool replace);

void UIDrawBlock(UIPainter *painter, UIRectangle rectangle, uint32_t color);
void UIDrawCircle(UIPainter *painter, int centerX, int centerY, int radius,
                  uint32_t fillColor, uint32_t outlineColor, bool hollow);
void UIDrawControl(UIPainter *painter, UIRectangle bounds,
                   uint32_t mode /* UI_DRAW_CONTROL_* */, const char *label,
                   ptrdiff_t labelBytes, double position, float scale);
void UIDrawControlDefault(UIPainter *painter, UIRectangle bounds, uint32_t mode,
                          const char *label, ptrdiff_t labelBytes,
...
void UIDrawTriangle(UIPainter *painter, int x0, int y0, int x1, int y1, int x2,
                    int y2, uint32_t color);
void UIDrawTriangleOutline(UIPainter *painter, int x0, int y0, int x1, int y1,
                           int x2, int y2, uint32_t color);
void UIDrawGlyph(UIPainter *painter, int x, int y, int c, uint32_t color);
void UIDrawRectangle(UIPainter *painter, UIRectangle r, uint32_t mainColor,
                     uint32_t borderColor, UIRectangle borderSize);
void UIDrawBorder(UIPainter *painter, UIRectangle r, uint32_t borderColor,
                  UIRectangle borderSize);
void UIDrawString(UIPainter *painter, UIRectangle r, const char *string,
                  ptrdiff_t bytes, uint32_t color, int align,
                  UIStringSelection *selection);
int UIDrawStringHighlighted(UIPainter *painter, UIRectangle r,
                            const char *string, ptrdiff_t bytes,
                            int tabSize); // Returns final x position.
...
UIRectangle UIElementScreenBounds(
    UIElement *element); // Returns bounds of element in same coordinate system
                         // as used by UIWindowCreate.
...
UIElement *UIElementChangeParent(
    UIElement *element, UIElement *newParent,
    UIElement *insertBefore); // Set insertBefore to null to insert at the end.
                              // Returns the element it was before in its
                              // previous parent, or NULL.
...
UIRectangle UIRectangleFit(UIRectangle parent, UIRectangle child,
                           bool allowScalingUp);
...
struct {
        ...
        Atom dndEnterID, dndPositionID, dndStatusID, dndActionCopyID, dndDropID,
            dndSelectionID, dndFinishedID, dndAwareID;
        ...
        void
            *menuData[256]; // HACK This limits the number of menu items to 128.
        uintptr_t menuIndex;
#endif
...
UIRectangle UIRectangleCenter(UIRectangle parent, UIRectangle child) {
    ...
    int parentWidth = UI_RECT_WIDTH(parent),
        parentHeight = UI_RECT_HEIGHT(parent);
    child.l = parentWidth / 2 - childWidth / 2 + parent.l,
    child.r = child.l + childWidth;
    child.t = parentHeight / 2 - childHeight / 2 + parent.t,
    child.b = child.t + childHeight;
    return child;
}
...
UIRectangle UIRectangleFit(UIRectangle parent, UIRectangle child,
    ...
    if (childMaximumWidth > parentWidth) {
        return UIRectangleCenter(parent,
                                 UI_RECT_2S(parentWidth, childMaximumHeight));
    } else {
        return UIRectangleCenter(parent,
                                 UI_RECT_2S(childMaximumWidth, parentHeight));
    }
}
...
float _UILinearMap(float value, float inFrom, float inTo, float outFrom,
                   float outTo) {
...
bool UIElementAnimate(UIElement *element, bool stop) {
    if (stop) {
        ...
        ui.animating = (UIElement **)UI_REALLOC(
            ui.animating, sizeof(UIElement *) * (ui.animatingCount + 1));
...
void UIDrawCircle(UIPainter *painter, int cx, int cy, int radius,
                  uint32_t fillColor, uint32_t outlineColor, bool hollow) {
        ...
        while (py <= iy) {
            if (py >= painter->clip.t && py < painter->clip.b) {
                for (int s = 0; s <= ix || s <= px; s++) {
                    bool inOutline =
                        ((s <= ix) != (s <= px)) || ((ix == px) && (s == ix));
                    if (hollow && !inOutline)
                        continue;
                    bool clip0 =
                        cx + s >= painter->clip.l && cx + s < painter->clip.r;
                    bool clip1 =
                        cx - s >= painter->clip.l && cx - s < painter->clip.r;
                    if (clip0)
...
#define _UI_DRAW_TRIANGLE_APPLY_CLIP(xo, yo)                                   \
    if (needsYClip &&                                                          \
        (yi + yo < painter->clip.t || yi + yo >= painter->clip.b))             \
        continue;                                                              \
...
int UIMeasureStringHeight() { return ui.activeFont->glyphHeight; }
...
void UIDrawString(UIPainter *painter, UIRectangle r, const char *string,
    ...
    for (; j < bytes; j++) {
        ...
        if (j >= selectFrom && j < selectTo) {
            UIDrawBlock(
                painter,
                UI_RECT_4(x, x + ui.activeFont->glyphWidth, y, y + height),
...
void UIDrawBorder(UIPainter *painter, UIRectangle r, uint32_t borderColor,
                  UIRectangle borderSize) {
    UIDrawBlock(painter, UI_RECT_4(r.l, r.r, r.t, r.t + borderSize.t),
                borderColor);
    UIDrawBlock(painter,
                UI_RECT_4(r.l, r.l + borderSize.l, r.t + borderSize.t,
                          r.b - borderSize.b),
                borderColor);
    UIDrawBlock(painter,
                UI_RECT_4(r.r - borderSize.r, r.r, r.t + borderSize.t,
                          r.b - borderSize.b),
                borderColor);
    UIDrawBlock(painter, UI_RECT_4(r.l, r.r, r.b - borderSize.b, r.b),
                borderColor);
}
...
void UIDrawRectangle(UIPainter *painter, UIRectangle r, uint32_t mainColor,
    ...
    UIDrawBlock(painter,
                UI_RECT_4(r.l + borderSize.l, r.r - borderSize.r,
                          r.t + borderSize.t, r.b - borderSize.b),
                mainColor);
}
...
void UIDrawControlDefault(UIPainter *painter, UIRectangle bounds, uint32_t mode,
    ...
    if (which == UI_DRAW_CONTROL_CHECKBOX) {
        ...
        UIRectangle boxBounds = UI_RECT_4(
            bounds.l, bounds.l + UI_SIZE_CHECKBOX_BOX,
            midY - UI_SIZE_CHECKBOX_BOX / 2, midY + UI_SIZE_CHECKBOX_BOX / 2);
        UIDrawRectangle(painter, boxBounds, color, ui.theme.border,
                        UI_RECT_1(1));
        ...
        UIDrawString(painter,
                     UIRectangleAdd(bounds, UI_RECT_4(UI_SIZE_CHECKBOX_BOX +
                                                          UI_SIZE_CHECKBOX_GAP,
                                                      0, 0, 0)),
                     label, labelBytes,
                     disabled ? ui.theme.textDisabled : ui.theme.text,
                     UI_ALIGN_LEFT, NULL);
    } else if (which == UI_DRAW_CONTROL_MENU_ITEM ||
               which == UI_DRAW_CONTROL_DROP_DOWN ||
               which == UI_DRAW_CONTROL_PUSH_BUTTON) {
        uint32_t color = buttonColor, textColor = buttonTextColor;
        int borderSize = which == UI_DRAW_CONTROL_MENU_ITEM ? 0 : scale;
        UIDrawRectangle(painter, bounds, color, ui.theme.border,
        ...
        if (checked && !focused) {
            UIDrawBlock(
                ...
                UIRectangleAdd(
                    bounds,
                    UI_RECT_1I((int)(UI_SIZE_BUTTON_CHECKED_AREA * scale))),
                ui.theme.buttonPressed);
        }

        UIRectangle innerBounds = UIRectangleAdd(
            bounds, UI_RECT_2I((int)(UI_SIZE_MENU_ITEM_MARGIN * scale), 0));
        ...
        if (which == UI_DRAW_CONTROL_MENU_ITEM) {
            ...
            UIDrawString(painter, innerBounds, label, tab, textColor,
                         UI_ALIGN_LEFT, NULL);

            if (labelBytes > tab) {
                UIDrawString(painter, innerBounds, label + tab + 1,
                             labelBytes - tab - 1, textColor, UI_ALIGN_RIGHT,
                             NULL);
            }
        } else if (which == UI_DRAW_CONTROL_DROP_DOWN) {
            UIDrawString(painter, innerBounds, label, labelBytes, textColor,
                         UI_ALIGN_LEFT, NULL);
            UIDrawString(painter, innerBounds, "\x19", 1, textColor,
                         UI_ALIGN_RIGHT, NULL);
        } else {
            UIDrawString(painter, bounds, label, labelBytes, textColor,
                         UI_ALIGN_CENTER, NULL);
        }
    } else if (which == UI_DRAW_CONTROL_LABEL) {
        UIDrawString(painter, bounds, label, labelBytes, ui.theme.text,
                     UI_ALIGN_LEFT, NULL);
    } else if (which == UI_DRAW_CONTROL_SPLITTER) {
        UIRectangle borders = (mode & UI_DRAW_CONTROL_STATE_VERTICAL)
                                  ? UI_RECT_2(0, 1)
                                  : UI_RECT_2(1, 0);
        UIDrawRectangle(painter, bounds, ui.theme.buttonNormal, ui.theme.border,
                        borders);
    } else if (which == UI_DRAW_CONTROL_SCROLL_TRACK) {
        if (disabled)
            UIDrawBlock(painter, bounds, ui.theme.panel1);
    } else if (which == UI_DRAW_CONTROL_SCROLL_DOWN ||
               which == UI_DRAW_CONTROL_SCROLL_UP) {
        ...
        if (mode & UI_DRAW_CONTROL_STATE_VERTICAL) {
            UIDrawGlyph(
                painter,
                (bounds.l + bounds.r - ui.activeFont->glyphWidth) / 2 + 1,
        ...
        uint32_t color = disabled  ? ui.theme.buttonDisabled
                         ...
                         : hovered ? ui.theme.buttonHovered
                                   : ui.theme.buttonNormal;
        UIRectangle thumb =
            vertical ? UI_RECT_4(center - (thumbSize + 1) / 2,
                                 center + thumbSize / 2,
                                 bounds.b - thumbPosition - thumbSize,
                                 bounds.b - thumbPosition)
                     : UI_RECT_4(bounds.l + thumbPosition,
        ...
        UIDrawBorder(painter, UIRectangleAdd(bounds2, UI_RECT_1(1)),
                     ui.theme.border, UI_RECT_1(1));
        ...
        UIDrawRectangle(
            painter,
            UI_RECT_4(bounds.l, bounds.r, bounds.t,
                      bounds.t + (int)(UI_SIZE_TABLE_HEADER * scale)),
        ...
        UIRectangle borders =
            UI_RECT_4(borderSize, borderSize, titleSize, borderSize);
        UIDrawBorder(painter, bounds, ui.theme.buttonNormal, borders);
        UIDrawBorder(painter, bounds, ui.theme.border, UI_RECT_1((int)scale));
        UIDrawBorder(painter, UIRectangleAdd(content, UI_RECT_1I(-1)),
                     ui.theme.border, UI_RECT_1((int)scale));
        UIDrawString(painter, title, label, labelBytes, ui.theme.text,
                     UI_ALIGN_LEFT, NULL);
    } else if (which == UI_DRAW_CONTROL_TAB) {
        uint32_t color =
            selected ? ui.theme.buttonPressed : ui.theme.buttonNormal;
        ...
        UIDrawRectangle(painter, bounds, ui.theme.panel1, ui.theme.border,
                        UI_RECT_4(0, 0, 0, 1));
    }
}
...
int UIElementMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message >= UI_MSG_INPUT_EVENTS_START &&
        message <= UI_MSG_INPUT_EVENTS_END &&
...
UIElement *UIElementChangeParent(UIElement *element, UIElement *newParent,
    ...
    for (uint32_t i = 0; i < element->parent->childCount; i++) {
        if (element->parent->children[i] == element) {
            UI_MEMMOVE(&element->parent->children[i],
                       &element->parent->children[i + 1],
                       sizeof(UIElement *) *
                           (element->parent->childCount - i - 1));
    ...
    for (uint32_t i = 0; i <= newParent->childCount; i++) {
        if (i == newParent->childCount ||
            newParent->children[i] == insertBefore) {
            newParent->children = (UIElement **)UI_REALLOC(
                newParent->children,
                sizeof(UIElement *) * (newParent->childCount + 1));
...
UIElement *UIElementCreate(size_t bytes, UIElement *parent, uint32_t flags,
    ...
    if (parent) {
        ...
        parent->children = (UIElement **)UI_REALLOC(
            parent->children, sizeof(UIElement *) * (parent->childCount + 1));
...
UIElement *UIParentPush(UIElement *element) {
    UI_ASSERT(ui.parentStackCount !=
              sizeof(ui.parentStack) / sizeof(ui.parentStack[0]));
...
int _UIPanelCalculatePerFill(UIPanel *panel, int *_count, int hSpace,
                             int vSpace, float scale) {
    ...
    for (uint32_t i = 0; i < panel->e.childCount; i++) {
        ...
        if (horizontal) {
            if (child->flags & UI_ELEMENT_H_FILL) {
                ...
                available -=
                    UIElementMessage(child, UI_MSG_GET_WIDTH, vSpace, 0);
            }
        } else {
            if (child->flags & UI_ELEMENT_V_FILL) {
                ...
                available -=
                    UIElementMessage(child, UI_MSG_GET_HEIGHT, hSpace, 0);
            }
        }
    }
...
int _UIPanelMeasure(UIPanel *panel, int di) {
    bool horizontal = panel->e.flags & UI_PANEL_HORIZONTAL;
    int perFill =
        _UIPanelCalculatePerFill(panel, NULL, horizontal ? di : 0,
                                 horizontal ? 0 : di, panel->e.window->scale);
    int size = 0;
    ...
    for (uint32_t i = 0; i < panel->e.childCount; i++) {
        ...
        int childSize = UIElementMessage(
            ...
            (child->flags &
             (horizontal ? UI_ELEMENT_H_FILL : UI_ELEMENT_V_FILL))
                ? perFill
                : 0,
            0);
        if (childSize > size)
    ...
    int border = horizontal ? (panel->border.t + panel->border.b)
                            : (panel->border.l + panel->border.r);
...
int _UIPanelLayout(UIPanel *panel, UIRectangle bounds, bool measure) {
    ...
    int vSpace =
        UI_RECT_HEIGHT(bounds) - UI_RECT_TOTAL_V(panel->border) * scale;
    int count = 0;
    int perFill =
        _UIPanelCalculatePerFill(panel, &count, hSpace, vSpace, scale);
    int scaledBorder2 = (horizontal ? panel->border.t : panel->border.l) *
...
int _UIPanelMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_LAYOUT) {
        int scrollBarWidth = panel->scrollBar
                                 ? (UI_SIZE_SCROLL_BAR * element->window->scale)
                                 : 0;
        ...
        if (horizontal) {
            ...
            int width = di && panel->scrollBar
                            ? (di - UI_SIZE_SCROLL_BAR * element->window->scale)
                            : di;
...
UIPanel *UIPanelCreate(UIElement *parent, uint32_t flags) {
    UIPanel *panel = (UIPanel *)UIElementCreate(sizeof(UIPanel), parent, flags,
                                                _UIPanelMessage, "Panel");
...
void _UIWrapPanelLayoutRow(UIWrapPanel *panel, uint32_t rowStart,
                           uint32_t rowEnd, int rowY, int rowHeight) {
    int rowPosition = 0;
    ...
    for (uint32_t i = rowStart; i < rowEnd; i++) {
        ...
        UIRectangle relative = UI_RECT_4(rowPosition, rowPosition + width,
                                         rowY + rowHeight / 2 - height / 2,
                                         rowY + rowHeight / 2 + height / 2);
        UIElementMove(child, UIRectangleTranslate(relative, panel->e.bounds),
                      false);
        rowPosition += width;
    }
}

int _UIWrapPanelMessage(UIElement *element, UIMessage message, int di,
                        void *dp) {
    ...
    if (message == UI_MSG_LAYOUT || message == UI_MSG_GET_HEIGHT) {
        ...
        int rowHeight = 0;
        int rowLimit =
            message == UI_MSG_LAYOUT ? UI_RECT_WIDTH(element->bounds) : di;
        ...
        for (uint32_t i = 0; i < panel->e.childCount; i++) {
            ...
            if (rowLimit && rowPosition + width > rowLimit) {
                _UIWrapPanelLayoutRow(panel, rowStart, i, totalHeight,
                                      rowHeight);
        ...
        if (message == UI_MSG_GET_HEIGHT) {
            ...
            _UIWrapPanelLayoutRow(panel, rowStart, panel->e.childCount,
                                  totalHeight, rowHeight);
        }
    }
...
UIWrapPanel *UIWrapPanelCreate(UIElement *parent, uint32_t flags) {
    return (UIWrapPanel *)UIElementCreate(sizeof(UIWrapPanel), parent, flags,
                                          _UIWrapPanelMessage, "Wrap Panel");
}

int _UISwitcherMessage(UIElement *element, UIMessage message, int di,
                       void *dp) {
...
UISwitcher *UISwitcherCreate(UIElement *parent, uint32_t flags) {
    return (UISwitcher *)UIElementCreate(sizeof(UISwitcher), parent, flags,
                                         _UISwitcherMessage, "Switcher");
}
...
int _UIButtonMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        int paddedSize =
            labelSize + UI_SIZE_BUTTON_PADDING * element->window->scale;
        if (isDropDown)
        ...
        UIDrawControl((UIPainter *)dp, element->bounds,
                      ...
                      button->label, button->labelBytes, 0,
                      element->window->scale);
...
UIButton *UIButtonCreate(UIElement *parent, uint32_t flags, const char *label,
                         ptrdiff_t labelBytes) {
    UIButton *button = (UIButton *)UIElementCreate(sizeof(UIButton), parent,
                                                   flags | UI_ELEMENT_TAB_STOP,
                                                   _UIButtonMessage, "Button");
    button->label = UIStringCopy(label, (button->labelBytes = labelBytes));
    return button;
}

int _UICheckboxMessage(UIElement *element, UIMessage message, int di,
                       void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        box->check =
            (box->check + 1) %
            ((element->flags & UI_CHECKBOX_ALLOW_INDETERMINATE) ? 3 : 2);
        UIElementRepaint(element, NULL);
        if (box->invoke)
...
UICheckbox *UICheckboxCreate(UIElement *parent, uint32_t flags,
    ...
    UICheckbox *box = (UICheckbox *)UIElementCreate(
        sizeof(UICheckbox), parent, flags | UI_ELEMENT_TAB_STOP,
...
int _UILabelMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        if (label->colorBackground) {
            UIDrawBlock(painter, element->bounds, label->colorBackground);
        }
        UIDrawString(painter, element->bounds, label->label, label->labelBytes,
                     label->colorText ? label->colorText : ui.theme.text,
                     UI_ALIGN_LEFT, NULL);
    } else if (message == UI_MSG_DEALLOCATE) {
        UI_FREE(label->label);
    }
...
void UILabelSetContent(UILabel *label, const char *string,
                       ptrdiff_t stringBytes) {
    UI_FREE(label->label);
...
UILabel *UILabelCreate(UIElement *parent, uint32_t flags, const char *string,
                       ptrdiff_t stringBytes) {
    UILabel *label = (UILabel *)UIElementCreate(sizeof(UILabel), parent, flags,
                                                _UILabelMessage, "Label");
    label->label = UIStringCopy(string, (label->labelBytes = stringBytes));
    return label;
}
...
int _UISplitPaneMessage(UIElement *element, UIMessage message, int di,
                        void *dp);

int _UISplitterMessage(UIElement *element, UIMessage message, int di,
                       void *dp) {
    ...
    if (message == UI_MSG_PAINT) {
        ...
        int cursor =
            vertical ? element->window->cursorY : element->window->cursorX;
        float scale = element->window->scale;
        UIRectangle bounds =
            UIRectangleAdd(splitPane->e.bounds,
                           UI_RECT_4((int)(splitPane->margin.l * scale),
                                     -(int)(splitPane->margin.r * scale),
                                     (int)(splitPane->margin.t * scale),
                                     -(int)(splitPane->margin.b * scale)));
        int splitterSize = UI_SIZE_SPLITTER * scale;
        int space =
            (vertical ? UI_RECT_HEIGHT(bounds) : UI_RECT_WIDTH(bounds)) -
            splitterSize;
        float oldWeight = splitPane->weight;
        splitPane->weight = (float)(cursor - splitterSize / 2 -
                                    (vertical ? bounds.t : bounds.l)) /
                            space;
...
int _UISplitPaneMessage(UIElement *element, UIMessage message, int di,
    ...
    if (message == UI_MSG_PAINT) {
        ...
        UIRectangle bounds = UIRectangleAdd(
            element->bounds, UI_RECT_4((int)(splitPane->margin.l * scale),
                                       -(int)(splitPane->margin.r * scale),
                                       (int)(splitPane->margin.t * scale),
                                       -(int)(splitPane->margin.b * scale)));
        ...
        if (vertical) {
            UIElementMove(
                left,
                UI_RECT_4(bounds.l, bounds.r, bounds.t, bounds.t + leftSize),
                false);
            UIElementMove(splitter,
                          UI_RECT_4(bounds.l, bounds.r, bounds.t + leftSize,
                                    bounds.t + leftSize + splitterSize),
                          false);
            UIElementMove(
                right,
                UI_RECT_4(bounds.l, bounds.r, bounds.b - rightSize, bounds.b),
                false);
        } else {
            UIElementMove(
                left,
                UI_RECT_4(bounds.l, bounds.l + leftSize, bounds.t, bounds.b),
                false);
            UIElementMove(splitter,
                          UI_RECT_4(bounds.l + leftSize,
                                    bounds.l + leftSize + splitterSize,
                                    bounds.t, bounds.b),
                          false);
            UIElementMove(
                right,
                UI_RECT_4(bounds.r - rightSize, bounds.r, bounds.t, bounds.b),
                false);
        }
    }
...
UISplitPane *UISplitPaneCreate(UIElement *parent, uint32_t flags,
                               float weight) {
    UISplitPane *splitPane = (UISplitPane *)UIElementCreate(
        sizeof(UISplitPane), parent, flags, _UISplitPaneMessage, "Split Pane");
    splitPane->weight = weight;
    UIElementCreate(sizeof(UIElement), &splitPane->e, 0, _UISplitterMessage,
                    "Splitter");
    return splitPane;
}
...
int _UITabPaneMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_PAINT) {
        ...
        UIDrawControl(painter, top, UI_DRAW_CONTROL_TAB_BAND, NULL, 0, 0,
                      element->window->scale);
            ...
            UIDrawControl(painter, tab,
                          UI_DRAW_CONTROL_TAB |
                              (tabPane->active == index
                                   ? UI_DRAW_CONTROL_STATE_SELECTED
                                   : 0),
                          tabPane->tabs + position, end - position, 0,
                          element->window->scale);
            tab.l = tab.r - 1;
        ...
        while (true) {
            ...
            if (UIRectangleContains(tab, element->window->cursorX,
        ...
        for (uint32_t index = 0; index < element->childCount; index++) {
            ...
            if (tabPane->active == index) {
                return baseHeight +
                       UIElementMessage(child, UI_MSG_GET_HEIGHT, di, dp);
            }
        }
    } else if (message == UI_MSG_DEALLOCATE) {
...
UITabPane *UITabPaneCreate(UIElement *parent, uint32_t flags,
                           const char *tabs) {
    UITabPane *tabPane = (UITabPane *)UIElementCreate(
        sizeof(UITabPane), parent, flags, _UITabPaneMessage, "Tab Pane");
...
UISpacer *UISpacerCreate(UIElement *parent, uint32_t flags, int width,
                         int height) {
    UISpacer *spacer = (UISpacer *)UIElementCreate(
        sizeof(UISpacer), parent, flags, _UISpacerMessage, "Spacer");
    spacer->width = width;
...
int _UIScrollBarMessage(UIElement *element, UIMessage message, int di,
                        void *dp) {
    ...
    if (message == UI_MSG_GET_WIDTH || message == UI_MSG_GET_HEIGHT) {
        ...
        if (scrollBar->page >= scrollBar->maximum || scrollBar->maximum <= 0 ||
            ...
            if (thumbSize <
                UI_SIZE_SCROLL_MINIMUM_THUMB * element->window->scale) {
                thumbSize =
                    UI_SIZE_SCROLL_MINIMUM_THUMB * element->window->scale;
            }
        ...
        UIDrawControl((UIPainter *)dp, element->bounds,
                      UI_DRAW_CONTROL_SCROLL_TRACK |
                          ((scrollBar->page >= scrollBar->maximum ||
...
int _UIScrollUpDownMessage(UIElement *element, UIMessage message, int di,
                           void *dp) {
...
int _UIScrollThumbMessage(UIElement *element, UIMessage message, int di,
                          void *dp) {
    UIScrollBar *scrollBar = (UIScrollBar *)element->parent;
    ...
    if (message == UI_MSG_PAINT) {
        ...
        UIElementRepaint(element, NULL);
               ...
               element->window->pressedButton == 1) {
        if (!scrollBar->inDrag) {
            scrollBar->inDrag = true;

            if (scrollBar->horizontal) {
                scrollBar->dragOffset = element->bounds.l -
                                        scrollBar->e.bounds.l -
                                        element->window->cursorX;
            } else {
                scrollBar->dragOffset = element->bounds.t -
                                        scrollBar->e.bounds.t -
                                        element->window->cursorY;
            }
        }

        int thumbPosition = (scrollBar->horizontal ? element->window->cursorX
                                                   : element->window->cursorY) +
                            scrollBar->dragOffset;
        int size = scrollBar->horizontal
                       ? (UI_RECT_WIDTH(scrollBar->e.bounds) -
                          UI_RECT_WIDTH(element->bounds))
                       : (UI_RECT_HEIGHT(scrollBar->e.bounds) -
                          UI_RECT_HEIGHT(element->bounds));
        scrollBar->position = (double)thumbPosition / size *
                              (scrollBar->maximum - scrollBar->page);
        UIElementRefresh(&scrollBar->e);
        UIElementMessage(scrollBar->e.parent, UI_MSG_SCROLLED, 0, 0);
    } else if (message == UI_MSG_LEFT_UP) {
...
UIScrollBar *UIScrollBarCreate(UIElement *parent, uint32_t flags) {
    UIScrollBar *scrollBar = (UIScrollBar *)UIElementCreate(
        sizeof(UIScrollBar), parent, flags, _UIScrollBarMessage, "Scroll Bar");
    bool horizontal = scrollBar->horizontal = flags & UI_SCROLL_BAR_HORIZONTAL;
    UIElementCreate(sizeof(UIElement), &scrollBar->e, flags,
                    _UIScrollUpDownMessage,
                    !horizontal ? "Scroll Up" : "Scroll Left")
        ->cp = (void *)(uintptr_t)0;
    UIElementCreate(sizeof(UIElement), &scrollBar->e, flags,
                    _UIScrollThumbMessage, "Scroll Thumb");
    UIElementCreate(sizeof(UIElement), &scrollBar->e, flags,
                    _UIScrollUpDownMessage,
...
bool _UICharIsDigit(char c) { return c >= '0' && c <= '9'; }
...
int UICodeHitTest(UICode *code, int x, int y) {
    ...
    if (x < 0 || x >= UI_RECT_WIDTH(code->e.bounds) -
                          UI_SIZE_SCROLL_BAR * code->e.window->scale) {
        return 0;
    }
...
int UIDrawStringHighlighted(UIPainter *painter, UIRectangle lineBounds,
                            const char *string, ptrdiff_t bytes, int tabSize) {
    if (bytes == -1)
        bytes = _UIStringLength(string);
    if (bytes > 10000)
        bytes = 10000;
    ...
    int ti = 0;
    int lexState = 0;
    bool inComment = false, inIdentifier = false, inChar = false,
         startedString = false;
    uint32_t last = 0;
...
int _UICodeMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_LAYOUT) {
        ...
        if (code->moveScrollToFocusNextLayout) {
            code->vScroll->position =
                (code->focused + 0.5) * UIMeasureStringHeight() -
                UI_RECT_HEIGHT(code->e.bounds) / 2;
        }

        UIRectangle scrollBarBounds = element->bounds;
        scrollBarBounds.l =
            scrollBarBounds.r - UI_SIZE_SCROLL_BAR * code->e.window->scale;
        ...
        for (int i = code->vScroll->position / lineHeight; i < code->lineCount;
             i++) {
            ...
            if (~code->e.flags & UI_CODE_NO_MARGIN) {
                ...
                uint32_t marginColor = UIElementMessage(
                    element, UI_MSG_CODE_GET_MARGIN_COLOR, i + 1, 0);
                ...
                UIDrawString(painter, marginBounds, string + p, 16 - p,
                             ui.theme.codeDefault, UI_ALIGN_RIGHT, NULL);
            }
            ...
            int x = UIDrawStringHighlighted(
                painter, lineBounds, code->content + code->lines[i].offset,
                code->lines[i].bytes, code->tabSize);
            int y = (lineBounds.t + lineBounds.b - UIMeasureStringHeight()) / 2;

            UICodeDecorateLine m = {0};
            m.x = x, m.y = y, m.bounds = lineBounds, m.index = i + 1,
            m.painter = painter;
        ...
        if (UICodeHitTest(code, element->window->cursorX,
                          element->window->cursorY) < 0) {
...
void UICodeInsertContent(UICode *code, const char *content, ptrdiff_t byteCount,
    ...
    code->content =
        (char *)UI_REALLOC(code->content, code->contentBytes + byteCount);
    ...
    code->lines = (UICodeLine *)UI_REALLOC(
        code->lines, sizeof(UICodeLine) * (code->lineCount + lineCount));
...
UICode *UICodeCreate(UIElement *parent, uint32_t flags) {
    UICode *code = (UICode *)UIElementCreate(sizeof(UICode), parent, flags,
                                             _UICodeMessage, "Code");
...
UIGauge *UIGaugeCreate(UIElement *parent, uint32_t flags) {
    return (UIGauge *)UIElementCreate(sizeof(UIGauge), parent, flags,
                                      _UIGaugeMessage, "Gauge");
}
...
int _UISliderMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        UIDrawControl((UIPainter *)dp, element->bounds,
                      UI_DRAW_CONTROL_SLIDER |
                          (vertical ? UI_DRAW_CONTROL_STATE_VERTICAL : 0) |
                          UI_DRAW_CONTROL_STATE_FROM_ELEMENT(element),
                      NULL, 0, slider->position, element->window->scale);
    } else if (message == UI_MSG_LEFT_DOWN ||
               (message == UI_MSG_MOUSE_DRAG &&
                element->window->pressedButton == 1)) {
        UIRectangle bounds = element->bounds;
        int thumbSize = UI_SIZE_SLIDER_THUMB * element->window->scale;
        double position = vertical ? 1 - ((float)(element->window->cursorY -
                                                  thumbSize / 2 - bounds.t) /
                                          (UI_RECT_HEIGHT(bounds) - thumbSize))
                                   : (double)(element->window->cursorX -
                                              thumbSize / 2 - bounds.l) /
...
void UISliderSetPosition(UISlider *slider, double position,
    ...
    if (position == slider->position)
        return;
    if (slider->steps > 1)
        position = (int)(position * (slider->steps - 1) + 0.5f) /
                   (float)(slider->steps - 1);
    if (position < 0)
        position = 0;
    if (position > 1)
        position = 1;
...
UISlider *UISliderCreate(UIElement *parent, uint32_t flags) {
    return (UISlider *)UIElementCreate(sizeof(UISlider), parent, flags,
                                       _UISliderMessage, "Slider");
}
...
int UITableHitTest(UITable *table, int x, int y) {
    ...
    if (x < 0 || x >= UI_RECT_WIDTH(table->e.bounds) -
                          UI_SIZE_SCROLL_BAR * table->e.window->scale) {
        return -1;
    }
...
int UITableHeaderHitTest(UITable *table, int x, int y) {
    ...
    while (true) {
        ...
        if (UIRectangleContains(header, x, y))
            return index;
        header.l += table->columnWidths[index] +
                    UI_SIZE_TABLE_COLUMN_GAP * table->e.window->scale;
...
bool UITableEnsureVisible(UITable *table, int index) {
    ...
    int height = UI_RECT_HEIGHT(table->e.bounds) -
                 UI_SIZE_TABLE_HEADER * table->e.window->scale - rowHeight;
...
void UITableResizeColumns(UITable *table) {
    ...
    while (true) {
        ...
        int longest =
            UIMeasureStringWidth(table->columns + position, end - position);

        for (int i = 0; i < table->itemCount; i++) {
            m.index = i;
            int bytes =
                UIElementMessage(&table->e, UI_MSG_TABLE_GET_ITEM, 0, &m);
...
int _UITableMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_PAINT) {
        ...
        int hovered = UITableHitTest(table, element->window->cursorX,
                                     element->window->cursorY);
        UIRectangle oldClip = painter->clip;
        painter->clip = UIRectangleIntersection(
            oldClip, UI_RECT_4(bounds.l, bounds.r,
                               bounds.t + (int)(UI_SIZE_TABLE_HEADER *
                                                element->window->scale),
                               bounds.b));

        for (int i = table->vScroll->position / rowHeight; i < table->itemCount;
             i++) {
            ...
            uint32_t rowFlags =
                (m.isSelected ? UI_DRAW_CONTROL_STATE_SELECTED : 0) |
                (hovered == i ? UI_DRAW_CONTROL_STATE_HOVERED : 0);
            UIDrawControl(painter, row, UI_DRAW_CONTROL_TABLE_ROW | rowFlags,
                          NULL, 0, 0, element->window->scale);
            ...
            for (int j = 0; j < table->columnCount; j++) {
                if (j) {
                    m.column = j;
                    bytes =
                        UIElementMessage(element, UI_MSG_TABLE_GET_ITEM, 0, &m);
                }

                cell.r = cell.l + table->columnWidths[j];
                if ((size_t)bytes > m.bufferBytes && bytes > 0)
                    bytes = m.bufferBytes;
                UIDrawControl(painter, cell,
                              UI_DRAW_CONTROL_TABLE_CELL | rowFlags, buffer,
                              bytes, 0, element->window->scale);
                cell.l += table->columnWidths[j] +
                          UI_SIZE_TABLE_COLUMN_GAP * table->e.window->scale;
            }
        ...
        if (table->columnCount) {
            while (true) {
                int end = position;
                for (; table->columns[end] != '\t' && table->columns[end];
                     end++)
                    ;
                ...
                UIDrawControl(painter, header,
                              UI_DRAW_CONTROL_TABLE_HEADER |
                                  (index == table->columnHighlight
                                       ? UI_DRAW_CONTROL_STATE_SELECTED
                                       : 0),
                              table->columns + position, end - position, 0,
                              element->window->scale);
                header.l += table->columnWidths[index] +
                            UI_SIZE_TABLE_COLUMN_GAP * table->e.window->scale;
        ...
        table->vScroll->maximum =
            table->itemCount * UI_SIZE_TABLE_ROW * element->window->scale;
        table->vScroll->page = UI_RECT_HEIGHT(element->bounds) -
                               UI_SIZE_TABLE_HEADER * table->e.window->scale;
...
UITable *UITableCreate(UIElement *parent, uint32_t flags, const char *columns) {
    UITable *table = (UITable *)UIElementCreate(sizeof(UITable), parent, flags,
                                                _UITableMessage, "Table");
...
void UITextboxReplace(UITextbox *textbox, const char *text, ptrdiff_t bytes,
    ...
    UI_MEMMOVE(&textbox->string[deleteFrom], &textbox->string[deleteTo],
               textbox->bytes - deleteTo);
    textbox->bytes -= deleteTo - deleteFrom;
    textbox->string =
        (char *)UI_REALLOC(textbox->string, textbox->bytes + bytes);
    UI_MEMMOVE(&textbox->string[deleteFrom + bytes],
               &textbox->string[deleteFrom], textbox->bytes - deleteFrom);
...
void UITextboxMoveCaret(UITextbox *textbox, bool backward, bool word) {
    while (true) {
        ...
        if (!word) {
            ...
            if (_UICharIsAlphaOrDigitOrUnderscore(c1) !=
                _UICharIsAlphaOrDigitOrUnderscore(c2)) {
                return;
            }
        }
    }
...
int _UITextboxMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        UIDrawControl(painter, element->bounds,
                      UI_DRAW_CONTROL_TEXTBOX |
                          UI_DRAW_CONTROL_STATE_FROM_ELEMENT(element),
                      NULL, 0, 0, element->window->scale);

        int scaledMargin = UI_SIZE_TEXTBOX_MARGIN * element->window->scale;
        int totalWidth = UIMeasureStringWidth(textbox->string, textbox->bytes) +
                         scaledMargin * 2;
        UIRectangle textBounds =
            UIRectangleAdd(element->bounds, UI_RECT_1I(scaledMargin));
        ...
        int caretX = UIMeasureStringWidth(textbox->string, textbox->carets[0]) -
                     textbox->scroll;
        ...
        if (caretX < 0) {
            ...
            textbox->scroll =
                caretX - UI_RECT_WIDTH(textBounds) + textbox->scroll + 1;
        }
        ...
        if (UI_RECT_VALID(painter->clip)) {
            ...
            int selectFrom = focused ? textbox->carets[0] : -1,
                selectTo = focused ? textbox->carets[1] : -1;
            if (selectFrom > selectTo) {
                UI_SWAP(int, selectFrom, selectTo);
            }
            ...
            for (; j < textbox->bytes; j++) {
                ...
                uint32_t colorText = (element->flags & UI_ELEMENT_DISABLED)
                                         ? ui.theme.textDisabled
                                         : ui.theme.text;

                if (j >= selectFrom && j < selectTo) {
                    UIDrawBlock(painter,
                                UI_RECT_4(x, x + ui.activeFont->glyphWidth, y,
                                          y + height),
                                ui.theme.selected);
                ...
                if (c != '\t') {
                    UIDrawGlyph(
                        painter, x, y,
                        element->flags & UI_TEXTBOX_HIDE_CHARACTERS ? '*' : c,
                        colorText);
                }
                if (focused && textbox->carets[0] == j) {
                    UIDrawInvert(painter, UI_RECT_4(x, x + 1, y, y + height));
                }
                x += ui.activeFont->glyphWidth, i++;
                if (c == '\t') {
        ...
        if (textbox->rejectNextKey) {
            ...
            handled = false;
        } else if (m->code == UI_KEYCODE_BACKSPACE ||
                   m->code == UI_KEYCODE_DELETE) {
            if (textbox->carets[0] == textbox->carets[1]) {
                UITextboxMoveCaret(textbox, m->code == UI_KEYCODE_BACKSPACE,
                                   element->window->ctrl);
            }
            ...
            if (textbox->carets[0] == textbox->carets[1] ||
                ...
                UITextboxMoveCaret(textbox, m->code == UI_KEYCODE_LEFT,
                                   element->window->ctrl);
            ...
            textbox->carets[1] = 0;
            textbox->carets[0] = textbox->bytes;
        } else if (m->textBytes && !element->window->alt &&
                   !element->window->ctrl && m->text[0] >= 0x20) {
            UITextboxReplace(textbox, m->text, m->textBytes, true);
        } else if ((m->code == UI_KEYCODE_LETTER('C') ||
                    m->code == UI_KEYCODE_LETTER('X') ||
                    m->code == UI_KEYCODE_INSERT) &&
                   element->window->ctrl && !element->window->alt &&
                   !element->window->shift) {
            int to = textbox->carets[0] > textbox->carets[1]
                         ? textbox->carets[0]
                         : textbox->carets[1];
            int from = textbox->carets[0] < textbox->carets[1]
                           ? textbox->carets[0]
                           : textbox->carets[1];
        ...
        } else if ((m->code == UI_KEYCODE_LETTER('V') &&
                    element->window->ctrl && !element->window->alt &&
                    !element->window->shift) ||
...
UITextbox *UITextboxCreate(UIElement *parent, uint32_t flags) {
    return (UITextbox *)UIElementCreate(sizeof(UITextbox), parent,
                                        flags | UI_ELEMENT_TAB_STOP,
...
int _UIMDIChildHitTest(UIMDIChild *mdiChild, int x, int y) {
    ...
    if (!UIRectangleContains(element->bounds, x, y) ||
        UIRectangleContains(content, x, y))
        return -1;
    else if (x < element->bounds.l + cornerSize &&
             y < element->bounds.t + cornerSize)
        return 0b1010;
    else if (x > element->bounds.r - cornerSize &&
             y < element->bounds.t + cornerSize)
        return 0b0110;
    else if (x < element->bounds.l + cornerSize &&
             y > element->bounds.b - cornerSize)
        return 0b1001;
    else if (x > element->bounds.r - cornerSize &&
             y > element->bounds.b - cornerSize)
        return 0b0101;
...
int _UIMDIChildMessage(UIElement *element, UIMessage message, int di,
                       void *dp) {
    UIMDIChild *mdiChild = (UIMDIChild *)element;

    if (message == UI_MSG_PAINT) {
        UIDrawControl((UIPainter *)dp, element->bounds,
                      UI_DRAW_CONTROL_MDI_CHILD, mdiChild->title,
                      mdiChild->titleBytes, 0, element->window->scale);
    } else if (message == UI_MSG_GET_WIDTH) {
        UIElement *child = element->childCount
                               ? element->children[element->childCount - 1]
                               : NULL;
        int width = 2 * UI_SIZE_MDI_CHILD_BORDER;
        width += (child ? UIElementMessage(child, message,
                                           di ? (di - UI_SIZE_MDI_CHILD_TITLE +
                                                 UI_SIZE_MDI_CHILD_BORDER)
                                              : 0,
                                           dp)
                        : 0);
        ...
        for (uint32_t i = 0; i < element->childCount - 1; i++) {
            ...
            int width = UIElementMessage(child, UI_MSG_GET_WIDTH, 0, 0);
            UIElementMove(
                child, UI_RECT_4(position - width, position, title.t, title.b),
                false);
            position -= width;
        }
        ...
        int hitTest = _UIMDIChildHitTest(mdiChild, element->window->cursorX,
                                         element->window->cursorY);
        if (hitTest == 0b1000)
        ...
        mdiChild->dragHitTest = _UIMDIChildHitTest(
            mdiChild, element->window->cursorX, element->window->cursorY);
        mdiChild->dragOffset = UIRectangleAdd(
            element->bounds,
            UI_RECT_2(-element->window->cursorX, -element->window->cursorY));
    } else if (message == UI_MSG_LEFT_UP) {
...
#define _UI_MDI_CHILD_MOVE_EDGE(bit, edge, cursor, size, opposite, negate,     \
                                minimum, offset)                               \
    if (mdiChild->dragHitTest & bit)                                           \
        mdiChild->bounds.edge = mdiChild->dragOffset.edge +                    \
                                element->window->cursor -                      \
                                element->parent->bounds.offset;                \
    if ((mdiChild->dragHitTest & bit) && size(mdiChild->bounds) < minimum)     \
        ...
        if (client->active == mdiChild) {
            client->active =
                (UIMDIChild *)(client->e.childCount == 1
                                   ? NULL
                                   : client->e
                                         .children[client->e.childCount - 2]);
        }
    } else if (message == UI_MSG_DEALLOCATE) {
...
int _UIMDIClientMessage(UIElement *element, UIMessage message, int di,
                        void *dp) {
    ...
    if (message == UI_MSG_PAINT) {
        ...
        for (uint32_t i = 0; i < element->childCount; i++) {
            ...
            if (UIRectangleEquals(mdiChild->bounds, UI_RECT_1(0))) {
                int width =
                    UIElementMessage(&mdiChild->e, UI_MSG_GET_WIDTH, 0, 0);
                int height =
                    UIElementMessage(&mdiChild->e, UI_MSG_GET_HEIGHT, width, 0);
                if (client->cascade + width > element->bounds.r ||
                    client->cascade + height > element->bounds.b)
                    client->cascade = 0;
                mdiChild->bounds =
                    UI_RECT_4(client->cascade, client->cascade + width,
                              client->cascade, client->cascade + height);
                client->cascade += UI_SIZE_MDI_CASCADE * element->window->scale;
            }

            UIRectangle bounds =
                UIRectangleAdd(mdiChild->bounds,
                               UI_RECT_2(element->bounds.l, element->bounds.t));
...
UIMDIChild *UIMDIChildCreate(UIElement *parent, uint32_t flags,
    ...
    UIMDIChild *mdiChild = (UIMDIChild *)UIElementCreate(
        sizeof(UIMDIChild), parent, flags, _UIMDIChildMessage, "MDIChild");
    ...
    if (flags & UI_MDI_CHILD_CLOSE_BUTTON) {
        UIButton *closeButton = UIButtonCreate(
            &mdiChild->e, UI_BUTTON_SMALL | UI_ELEMENT_NON_CLIENT, "X", 1);
...
UIMDIClient *UIMDIClientCreate(UIElement *parent, uint32_t flags) {
    return (UIMDIClient *)UIElementCreate(sizeof(UIMDIClient), parent, flags,
                                          _UIMDIClientMessage, "MDIClient");
}
...
void _UIImageDisplayUpdateViewport(UIImageDisplay *display) {
    ...
    float minimumZoom =
        minimumZoomX < minimumZoomY ? minimumZoomX : minimumZoomY;

    if (display->zoom < minimumZoom ||
...
int _UIImageDisplayMessage(UIElement *element, UIMessage message, int di,
                           void *dp) {
    ...
    if (message == UI_MSG_GET_HEIGHT) {
        ...
        int w = UI_RECT_WIDTH(element->bounds),
            h = UI_RECT_HEIGHT(element->bounds);
        int x = _UILinearMap(0, display->panX,
                             display->panX + w / display->zoom, 0, w) +
                element->bounds.l;
        int y = _UILinearMap(0, display->panY,
                             display->panY + h / display->zoom, 0, h) +
                element->bounds.t;

        UIRectangle image =
            UI_RECT_4(x, x + (int)(display->width * display->zoom), y,
        ...
        if (display->zoom == 1) {
            uint32_t *lineStart = (uint32_t *)painter->bits +
                                  bounds.t * painter->width + bounds.l;
            uint32_t *sourceLineStart = display->bits + (bounds.l - image.l) +
                                        display->width * (bounds.t - image.t);

            for (int i = 0; i < bounds.b - bounds.t; i++,
                     lineStart += painter->width,
                     sourceLineStart += display->width) {
            ...
            for (int i = bounds.t; i < bounds.b; i++) {
                ...
                for (int j = bounds.l; j < bounds.r; j++) {
                    int tx = (j - image.l) * zr;
                    destination[i * painter->width + j] =
                        display->bits[ty * display->width + tx];
                }
            }
        }
    } else if (message == UI_MSG_MOUSE_WHEEL &&
               (element->flags & UI_IMAGE_DISPLAY_INTERACTIVE)) {
        display->e.flags &= ~_UI_IMAGE_DISPLAY_ZOOM_FIT;
        int divisions = -di / 72;
        float factor = 1;
        float perDivision = element->window->ctrl  ? 2.0f
                            : element->window->alt ? 1.01f
                                                   : 1.2f;
        while (divisions > 0)
        ...
        bounds.r -= bounds.l, bounds.b -= bounds.t;
        display->panX -=
            (bounds.r - display->previousWidth) / 2 / display->zoom;
        display->panY -=
            (bounds.b - display->previousHeight) / 2 / display->zoom;
        display->previousWidth = bounds.r, display->previousHeight = bounds.b;
        _UIImageDisplayUpdateViewport(display);
    } else if (message == UI_MSG_GET_CURSOR &&
               (element->flags & UI_IMAGE_DISPLAY_INTERACTIVE) &&
               (UI_RECT_WIDTH(element->bounds) <
                    display->width * display->zoom ||
                UI_RECT_HEIGHT(element->bounds) <
                    display->height * display->zoom)) {
        return UI_CURSOR_HAND;
    } else if (message == UI_MSG_MOUSE_DRAG) {
        display->panX -=
            (element->window->cursorX - display->previousPanPointX) /
            display->zoom;
        display->panY -=
            (element->window->cursorY - display->previousPanPointY) /
            display->zoom;
        _UIImageDisplayUpdateViewport(display);
...
UIImageDisplay *UIImageDisplayCreate(UIElement *parent, uint32_t flags,
    ...
    UIImageDisplay *display = (UIImageDisplay *)UIElementCreate(
        sizeof(UIImageDisplay), parent, flags, _UIImageDisplayMessage,
        "ImageDisplay");
    display->zoom = 1.0f;
...
int _UIDialogWrapperMessage(UIElement *element, UIMessage message, int di,
                            void *dp) {
    if (message == UI_MSG_LAYOUT) {
        int width =
            UIElementMessage(element->children[0], UI_MSG_GET_WIDTH, 0, 0);
        int height =
            UIElementMessage(element->children[0], UI_MSG_GET_HEIGHT, width, 0);
        int cx = (element->bounds.l + element->bounds.r) / 2;
        int cy = (element->bounds.t + element->bounds.b) / 2;
        UIRectangle bounds = UI_RECT_4(cx - (width + 1) / 2, cx + width / 2,
                                       cy - (height + 1) / 2, cy + height / 2);
        UIElementMove(element->children[0], bounds, false);
        UIElementRepaint(element, NULL);
    } else if (message == UI_MSG_PAINT) {
        UIDrawControl((UIPainter *)dp, element->children[0]->bounds,
                      UI_DRAW_CONTROL_MODAL_POPUP, NULL, 0, 0,
                      element->window->scale);
    } else if (message == UI_MSG_KEY_TYPED) {
        ...
        if (typed->textBytes == 1 && typed->text[0] >= 'a' &&
            typed->text[0] <= 'z') {
        ...
        for (uint32_t i = 0; i < rowContainer->childCount; i++) {
            for (uint32_t j = 0; j < rowContainer->children[i]->childCount;
                 j++) {
...
void _UIDialogButtonInvoke(void *cp) { ui.dialogResult = (const char *)cp; }

int _UIDialogDefaultButtonMessage(UIElement *element, UIMessage message, int di,
                                  void *dp) {
    if (message == UI_MSG_PAINT &&
        element->window->focused->messageClass != _UIButtonMessage) {
        element->flags |= UI_BUTTON_CHECKED;
...
int _UIDialogTextboxMessage(UIElement *element, UIMessage message, int di,
                            void *dp) {
    UITextbox *textbox = (UITextbox *)element;
...
const char *UIDialogShow(UIWindow *window, uint32_t flags, const char *format,
    ...
    window->dialog = UIElementCreate(sizeof(UIElement), &window->e, 0,
                                     _UIDialogWrapperMessage, "DialogWrapper");
    UIPanel *panel = UIPanelCreate(window->dialog,
                                   UI_PANEL_MEDIUM_SPACING | UI_PANEL_COLOR_1);
    ...
    for (int i = 0; format[i]; i++) {
        if (i == 0 || format[i - 1] == '\n') {
            row = UIPanelCreate(&panel->e,
                                UI_PANEL_HORIZONTAL | UI_ELEMENT_H_FILL);
        ...
        if (format[i] == ' ' || format[i] == '\n') {
            ...
            if (format[i] == 'b' /* button */ ||
                ...
                UITextbox *textbox =
                    UITextboxCreate(&row->e, UI_ELEMENT_H_FILL);
                if (!focus)
                    focus = &textbox->e;
                if (*buffer)
                    UITextboxReplace(textbox, *buffer, _UIStringLength(*buffer),
                                     false);
                textbox->e.cp = buffer;
                ...
                UISpacerCreate(&row->e, UI_ELEMENT_BORDER | UI_ELEMENT_H_FILL,
                               0, 1);
            } else if (format[i] == 'u' /* user */) {
                UIDialogUserCallback callback =
                    va_arg(arguments, UIDialogUserCallback);
                callback(&row->e);
            }
        } else {
            int j = i;
    ...
    if (!ui.dialogResult) {
               ...
               ui.dialogResult[2] == 'C' && ui.dialogResult[3] == 0 &&
               cancelButton) {
        ui.dialogResult = (const char *)cancelButton->e.cp;
    } else if (ui.dialogResult[0] == '_' && ui.dialogResult[1] == '_' &&
               ui.dialogResult[2] == 'D' && ui.dialogResult[3] == 0 &&
               defaultButton) {
...
#ifndef UI_ESSENCE
int _UIMenuItemMessage(UIElement *element, UIMessage message, int di,
                       void *dp) {
...
int _UIMenuMessage(UIElement *element, UIMessage message, int di, void *dp) {
    ...
    if (message == UI_MSG_GET_WIDTH) {
        ...
        UIDrawControl((UIPainter *)dp, element->bounds, UI_DRAW_CONTROL_MENU,
                      NULL, 0, 0, element->window->scale);
    } else if (message == UI_MSG_LAYOUT) {
        int position = element->bounds.t + 2 - menu->vScroll->position;
        int totalHeight = 0;
        int scrollBarSize =
            (menu->e.flags & UI_MENU_NO_SCROLL) ? 0 : UI_SIZE_SCROLL_BAR;
            ...
            if (~child->flags & UI_ELEMENT_NON_CLIENT) {
                ...
                UIElementMove(child,
                              UI_RECT_4(element->bounds.l + 2,
                                        element->bounds.r - scrollBarSize - 2,
        ...
        UIRectangle scrollBarBounds = element->bounds;
        scrollBarBounds.l =
            scrollBarBounds.r - scrollBarSize * element->window->scale;
...
void UIMenuAddItem(UIMenu *menu, uint32_t flags, const char *label,
                   ptrdiff_t labelBytes, void (*invoke)(void *cp), void *cp) {
    UIButton *button = UIButtonCreate(&menu->e, flags | UI_BUTTON_MENU_ITEM,
                                      label, labelBytes);
...
UIMenu *UIMenuCreate(UIElement *parent, uint32_t flags) {
    ...
    UIMenu *menu = (UIMenu *)UIElementCreate(sizeof(UIMenu), &window->e, flags,
                                             _UIMenuMessage, "Menu");
    ...
    if (parent->parent) {
        ...
        menu->pointY = (flags & UI_MENU_PLACE_ABOVE) ? (screenBounds.t + 1)
                                                     : (screenBounds.b - 1);
    } else {
        int x = 0, y = 0;
...
void UIWindowRegisterShortcut(UIWindow *window, UIShortcut shortcut) {
    if (window->shortcutCount + 1 > window->shortcutAllocated) {
        ...
        window->shortcuts = (UIShortcut *)UI_REALLOC(
            window->shortcuts, window->shortcutAllocated * sizeof(UIShortcut));
    }
...
void UIElementMeasurementsChanged(UIElement *element, int which) {
    ...
    while (true) {
        if (element->parent->flags & UI_ELEMENT_DESTROY)
            return;
        which &= ~UIElementMessage(element->parent, UI_MSG_GET_CHILD_STABILITY,
                                   which, element);
        if (!which)
            break;
        element->flags |= UI_ELEMENT_RELAYOUT;
...
void UIElementRepaint(UIElement *element, UIRectangle *region) {
    ...
    if (UI_RECT_VALID(element->window->updateRegion)) {
        element->window->updateRegion =
            UIRectangleBounding(element->window->updateRegion, r);
    } else {
...
void UIElementMove(UIElement *element, UIRectangle bounds, bool layout) {
    UIRectangle clip =
        element->parent ? UIRectangleIntersection(element->parent->clip, bounds)
                        : bounds;
    bool moved = !UIRectangleEquals(element->bounds, bounds) ||
                 !UIRectangleEquals(element->clip, clip);
    ...
    if (layout) {
        ...
        for (uint32_t i = 0; i < element->childCount; i++) {
            UIElementMove(element->children[i], element->children[i]->bounds,
                          false);
        }
    }
    ...
    while (window) {
        ...
        if (_UIDestroy(&window->e)) {
            *link = next;
            ...
            UIElementMessage(&window->e, UI_MSG_WINDOW_UPDATE_BEFORE_LAYOUT, 0,
                             0);
            UIElementMove(&window->e, window->e.bounds, false);
            UIElementMessage(&window->e, UI_MSG_WINDOW_UPDATE_BEFORE_PAINT, 0,
                             0);
            ...
            if (UI_RECT_VALID(window->updateRegion)) {
                ...
                window->lastFullFillCount =
                    ...
                    (UI_RECT_WIDTH(window->updateRegion) *
                     UI_RECT_HEIGHT(window->updateRegion));
#endif
            }
...
UIElement *UIElementFindByPoint(UIElement *element, int x, int y) {
    for (uint32_t i = element->childCount; i > 0; i--) {
        ...
        if ((~child->flags & UI_ELEMENT_HIDE) &&
            UIRectangleContains(child->clip, x, y)) {
    ...
    for (uint32_t i = 0; i < element->parent->childCount; i++) {
        if (element->parent->children[i] == element) {
            if (previous) {
                ...
                return i < element->parent->childCount - 1
                           ? element->parent->children[i + 1]
                           : NULL;
            }
        }
...
bool _UIWindowInputEvent(UIWindow *window, UIMessage message, int di,
                         void *dp) {
    bool handled = true;
    ...
    if (window->pressed) {
        bool inside = UIRectangleContains(window->pressed->clip,
                                          window->cursorX, window->cursorY);

        if (inside && window->hovered == &window->e) {
            window->hovered = window->pressed;
            UIElementMessage(window->pressed, UI_MSG_UPDATE, UI_UPDATE_HOVERED,
                             0);
    ...
    if (!window->pressed) {
        UIElement *hovered =
            UIElementFindByPoint(&window->e, window->cursorX, window->cursorY);

        if (message == UI_MSG_MOUSE_MOVE) {
            UIElementMessage(hovered, UI_MSG_MOUSE_MOVE, di, dp);

            int cursor =
                UIElementMessage(window->hovered, UI_MSG_GET_CURSOR, di, dp);
            ...
            if (!handled && !UIMenusOpen() && message == UI_MSG_KEY_TYPED) {
                ...
                if (m->code == UI_KEYCODE_TAB && !window->ctrl &&
                    !window->alt) {
                    UIElement *start =
                        window->focused ? window->focused : &window->e;
                    UIElement *element = start;

                    do {
                        if (element->childCount &&
                            !(element->flags &
                              (UI_ELEMENT_HIDE | UI_ELEMENT_DISABLED))) {
                            element =
                                window->shift
                                    ? element->children[element->childCount - 1]
                                    : element->children[0];
                        ...
                        while (element) {
                            UIElement *sibling =
                                _UIElementNextOrPreviousSibling(element,
                                                                window->shift);
                            if (sibling) {
                                element = sibling;
                                break;
                    ...
                    for (intptr_t i = window->shortcutCount - 1; i >= 0; i--) {
                        ...
                        if (shortcut->code == m->code &&
                            shortcut->ctrl == window->ctrl &&
                            shortcut->shift == window->shift &&
        ...
        if (hovered != window->hovered) {
            ...
            UIElementMessage(window->hovered, UI_MSG_UPDATE, UI_UPDATE_HOVERED,
                             0);
        }
    }
...
const uint64_t _uiFont[] = {
    0x0000000000000000UL, 0x0000000000000000UL, 0xBD8181A5817E0000UL,
    0x000000007E818199UL, 0xC3FFFFDBFF7E0000UL, 0x000000007EFFFFE7UL,
    0x7F7F7F3600000000UL, 0x00000000081C3E7FUL, 0x7F3E1C0800000000UL,
    0x0000000000081C3EUL, 0xE7E73C3C18000000UL, 0x000000003C1818E7UL,
    0xFFFF7E3C18000000UL, 0x000000003C18187EUL, 0x3C18000000000000UL,
    0x000000000000183CUL, 0xC3E7FFFFFFFFFFFFUL, 0xFFFFFFFFFFFFE7C3UL,
    0x42663C0000000000UL, 0x00000000003C6642UL, 0xBD99C3FFFFFFFFFFUL,
    0xFFFFFFFFFFC399BDUL, 0x331E4C5870780000UL, 0x000000001E333333UL,
    0x3C666666663C0000UL, 0x0000000018187E18UL, 0x0C0C0CFCCCFC0000UL,
    0x00000000070F0E0CUL, 0xC6C6C6FEC6FE0000UL, 0x0000000367E7E6C6UL,
    0xE73CDB1818000000UL, 0x000000001818DB3CUL, 0x1F7F1F0F07030100UL,
    0x000000000103070FUL, 0x7C7F7C7870604000UL, 0x0000000040607078UL,
    0x1818187E3C180000UL, 0x0000000000183C7EUL, 0x6666666666660000UL,
    0x0000000066660066UL, 0xD8DEDBDBDBFE0000UL, 0x00000000D8D8D8D8UL,
    0x6363361C06633E00UL, 0x0000003E63301C36UL, 0x0000000000000000UL,
    0x000000007F7F7F7FUL, 0x1818187E3C180000UL, 0x000000007E183C7EUL,
    0x1818187E3C180000UL, 0x0000000018181818UL, 0x1818181818180000UL,
    0x00000000183C7E18UL, 0x7F30180000000000UL, 0x0000000000001830UL,
    0x7F060C0000000000UL, 0x0000000000000C06UL, 0x0303000000000000UL,
    0x0000000000007F03UL, 0xFF66240000000000UL, 0x0000000000002466UL,
    0x3E1C1C0800000000UL, 0x00000000007F7F3EUL, 0x3E3E7F7F00000000UL,
    0x0000000000081C1CUL, 0x0000000000000000UL, 0x0000000000000000UL,
    0x18183C3C3C180000UL, 0x0000000018180018UL, 0x0000002466666600UL,
    0x0000000000000000UL, 0x36367F3636000000UL, 0x0000000036367F36UL,
    0x603E0343633E1818UL, 0x000018183E636160UL, 0x1830634300000000UL,
    0x000000006163060CUL, 0x3B6E1C36361C0000UL, 0x000000006E333333UL,
    0x000000060C0C0C00UL, 0x0000000000000000UL, 0x0C0C0C0C18300000UL,
    0x0000000030180C0CUL, 0x30303030180C0000UL, 0x000000000C183030UL,
    0xFF3C660000000000UL, 0x000000000000663CUL, 0x7E18180000000000UL,
    0x0000000000001818UL, 0x0000000000000000UL, 0x0000000C18181800UL,
    0x7F00000000000000UL, 0x0000000000000000UL, 0x0000000000000000UL,
    0x0000000018180000UL, 0x1830604000000000UL, 0x000000000103060CUL,
    0xDBDBC3C3663C0000UL, 0x000000003C66C3C3UL, 0x1818181E1C180000UL,
    0x000000007E181818UL, 0x0C183060633E0000UL, 0x000000007F630306UL,
    0x603C6060633E0000UL, 0x000000003E636060UL, 0x7F33363C38300000UL,
    0x0000000078303030UL, 0x603F0303037F0000UL, 0x000000003E636060UL,
    0x633F0303061C0000UL, 0x000000003E636363UL, 0x18306060637F0000UL,
    0x000000000C0C0C0CUL, 0x633E6363633E0000UL, 0x000000003E636363UL,
    0x607E6363633E0000UL, 0x000000001E306060UL, 0x0000181800000000UL,
    0x0000000000181800UL, 0x0000181800000000UL, 0x000000000C181800UL,
    0x060C183060000000UL, 0x000000006030180CUL, 0x00007E0000000000UL,
    0x000000000000007EUL, 0x6030180C06000000UL, 0x00000000060C1830UL,
    0x18183063633E0000UL, 0x0000000018180018UL, 0x7B7B63633E000000UL,
    0x000000003E033B7BUL, 0x7F6363361C080000UL, 0x0000000063636363UL,
    0x663E6666663F0000UL, 0x000000003F666666UL, 0x03030343663C0000UL,
    0x000000003C664303UL, 0x66666666361F0000UL, 0x000000001F366666UL,
    0x161E1646667F0000UL, 0x000000007F664606UL, 0x161E1646667F0000UL,
    0x000000000F060606UL, 0x7B030343663C0000UL, 0x000000005C666363UL,
    0x637F636363630000UL, 0x0000000063636363UL, 0x18181818183C0000UL,
    0x000000003C181818UL, 0x3030303030780000UL, 0x000000001E333333UL,
    0x1E1E366666670000UL, 0x0000000067666636UL, 0x06060606060F0000UL,
    0x000000007F664606UL, 0xC3DBFFFFE7C30000UL, 0x00000000C3C3C3C3UL,
    0x737B7F6F67630000UL, 0x0000000063636363UL, 0x63636363633E0000UL,
    0x000000003E636363UL, 0x063E6666663F0000UL, 0x000000000F060606UL,
    0x63636363633E0000UL, 0x000070303E7B6B63UL, 0x363E6666663F0000UL,
    0x0000000067666666UL, 0x301C0663633E0000UL, 0x000000003E636360UL,
    0x18181899DBFF0000UL, 0x000000003C181818UL, 0x6363636363630000UL,
    0x000000003E636363UL, 0xC3C3C3C3C3C30000UL, 0x00000000183C66C3UL,
    0xDBC3C3C3C3C30000UL, 0x000000006666FFDBUL, 0x18183C66C3C30000UL,
    0x00000000C3C3663CUL, 0x183C66C3C3C30000UL, 0x000000003C181818UL,
    0x0C183061C3FF0000UL, 0x00000000FFC38306UL, 0x0C0C0C0C0C3C0000UL,
    0x000000003C0C0C0CUL, 0x1C0E070301000000UL, 0x0000000040607038UL,
    0x30303030303C0000UL, 0x000000003C303030UL, 0x0000000063361C08UL,
    0x0000000000000000UL, 0x0000000000000000UL, 0x0000FF0000000000UL,
    0x0000000000180C0CUL, 0x0000000000000000UL, 0x3E301E0000000000UL,
    0x000000006E333333UL, 0x66361E0606070000UL, 0x000000003E666666UL,
    0x03633E0000000000UL, 0x000000003E630303UL, 0x33363C3030380000UL,
    0x000000006E333333UL, 0x7F633E0000000000UL, 0x000000003E630303UL,
    0x060F0626361C0000UL, 0x000000000F060606UL, 0x33336E0000000000UL,
    0x001E33303E333333UL, 0x666E360606070000UL, 0x0000000067666666UL,
    0x18181C0018180000UL, 0x000000003C181818UL, 0x6060700060600000UL,
    0x003C666660606060UL, 0x1E36660606070000UL, 0x000000006766361EUL,
    0x18181818181C0000UL, 0x000000003C181818UL, 0xDBFF670000000000UL,
    0x00000000DBDBDBDBUL, 0x66663B0000000000UL, 0x0000000066666666UL,
    0x63633E0000000000UL, 0x000000003E636363UL, 0x66663B0000000000UL,
    0x000F06063E666666UL, 0x33336E0000000000UL, 0x007830303E333333UL,
    0x666E3B0000000000UL, 0x000000000F060606UL, 0x06633E0000000000UL,
    ...
    0x00000000183C66C3UL, 0xC3C3C30000000000UL, 0x0000000066FFDBDBUL,
    0x3C66C30000000000UL, 0x00000000C3663C18UL, 0x6363630000000000UL,
    0x001F30607E636363UL, 0x18337F0000000000UL, 0x000000007F63060CUL,
    0x180E181818700000UL, 0x0000000070181818UL, 0x1800181818180000UL,
    0x0000000018181818UL, 0x18701818180E0000UL, 0x000000000E181818UL,
    0x000000003B6E0000UL, 0x0000000000000000UL, 0x63361C0800000000UL,
...
void UIDrawGlyph(UIPainter *painter, int x0, int y0, int c, uint32_t color) {
    ...
    if (font->isFreeType) {
        ...
        if (!font->glyphsRendered[c]) {
            ...
            font->glyphOffsetsY[c] = font->font->size->metrics.ascender / 64 -
                                     font->font->glyph->bitmap_top;
        ...
        for (int y = 0; y < (int)bitmap->rows; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                uint32_t *destination =
                    painter->bits + (x0 + x) + (y0 + y) * painter->width;
                uint32_t original = *destination;

#ifdef UI_FREETYPE_SUBPIXEL
                uint32_t ra =
                    ((uint8_t *)bitmap->buffer)[x * 3 + y * bitmap->pitch + 0];
                uint32_t ga =
                    ((uint8_t *)bitmap->buffer)[x * 3 + y * bitmap->pitch + 1];
                uint32_t ba =
                    ((uint8_t *)bitmap->buffer)[x * 3 + y * bitmap->pitch + 2];
                ra += (ga - ra) / 2, ba += (ga - ba) / 2;
#else
                uint32_t ra =
                    ((uint8_t *)bitmap->buffer)[x + y * bitmap->pitch];
    ...
    UIRectangle rectangle = UIRectangleIntersection(
        painter->clip, UI_RECT_4(x0, x0 + 8, y0, y0 + 16));
...
UIFont *UIFontCreate(const char *cPath, uint32_t size) {
    ...
    if (cPath) {
        if (!FT_New_Face(ui.ft, cPath, 0, &font->font)) {
            ...
            font->glyphHeight = (font->font->size->metrics.ascender -
                                 font->font->size->metrics.descender) /
                                64;
...
UIElement *_UIInspectorFindNthElement(UIElement *element, int *index,
                                      int *depth) {
    if (*index == 0) {
        return element;
    }
...
int _UIInspectorTableMessage(UIElement *element, UIMessage message, int di,
                             void *dp) {
    ...
    if (message == UI_MSG_TABLE_GET_ITEM) {
        ...
        int depth = 0;
        UIElement *element =
            _UIInspectorFindNthElement(&ui.inspectorTarget->e, &index, &depth);
        if (!element)
            return 0;
        ...
        int index = UITableHitTest(ui.inspectorTable, element->window->cursorX,
                                   element->window->cursorY);
        UIElement *element = NULL;
        if (index >= 0)
            element = _UIInspectorFindNthElement(&ui.inspectorTarget->e, &index,
                                                 NULL);
        ...
        if (element) {
            ...
            UIDrawInvert(&painter,
                         UIRectangleAdd(element->bounds, UI_RECT_1I(4)));
        }
...
void _UIInspectorRefresh() {
    if (!ui.inspectorTarget || !ui.inspector || !ui.inspectorTable)
        return;
    ui.inspectorTable->itemCount =
        _UIInspectorCountElements(&ui.inspectorTarget->e);
...
void UIAutomationKeyboardTypeSingle(intptr_t code, bool ctrl, bool shift,
                                    bool alt) {
...
bool UIAutomationCheckCodeLineMatches(UICode *code, int lineIndex,
                                      const char *input) {
...
bool UIAutomationCheckTableItemMatches(UITable *table, int row, int column,
                                       const char *input) {
    int bytes = 0;
    for (int i = 0; input[i]; i++)
        bytes++;
...
int _UIWindowMessageCommon(UIElement *element, UIMessage message, int di,
                           void *dp) {
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle,
    ...
    UIWindow *window = (UIWindow *)UIElementCreate(sizeof(UIWindow), NULL,
                                                   flags | UI_ELEMENT_WINDOW,
                                                   _UIWindowMessage, "Window");
    _UIWindowAdd(window);
    if (owner)
    ...
    window->window = XCreateWindow(
        ui.display, DefaultRootWindow(ui.display), 0, 0, width, height, 0, 0,
    ...
    if (flags & UI_WINDOW_MAXIMIZE) {
        Atom atoms[2] = {
            XInternAtom(ui.display, "_NET_WM_STATE_MAXIMIZED_HORZ", 0),
            XInternAtom(ui.display, "_NET_WM_STATE_MAXIMIZED_VERT", 0)};
        XChangeProperty(ui.display, window->window,
                        XInternAtom(ui.display, "_NET_WM_STATE", 0), XA_ATOM,
    ...
    if (flags & UI_WINDOW_CENTER_IN_OWNER) {
        ...
        XMoveResizeWindow(ui.display, window->window,
    ...
    window->image = XCreateImage(ui.display, ui.visual, 24, ZPixmap, 0, NULL,
                                 10, 10, 32, 0);

    window->xic = XCreateIC(
        ui.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
        XNClientWindow, window->window, XNFocusWindow, window->window, NULL);

    int dndVersion = 4;
    XChangeProperty(ui.display, window->window, ui.dndAwareID, XA_ATOM,
                    32 /* bits */, PropModeReplace, (uint8_t *)&dndVersion, 1);

    return window;
}

Display *_UIX11GetDisplay() { return ui.display; }
...
char *_UIClipboardReadTextStart(UIWindow *window, size_t *bytes) {
    ...
    XConvertSelection(ui.display, ui.clipboardID, XA_STRING,
                      ui.xSelectionDataID, window->window, CurrentTime);
    XSync(ui.display, 0);
    XNextEvent(ui.display, &ui.copyEvent);

    // Hack to get around the fact that PropertyNotify arrives before
    // SelectionNotify. We need PropertyNotify for incremental transfers.
    while (ui.copyEvent.type == PropertyNotify) {
        XNextEvent(ui.display, &ui.copyEvent);
    }

    if (ui.copyEvent.type == SelectionNotify &&
        ui.copyEvent.xselection.selection == ui.clipboardID &&
        ...
        int format;
        XGetWindowProperty(
            ui.copyEvent.xselection.display, ui.copyEvent.xselection.requestor,
            ui.copyEvent.xselection.property, 0L, ~0L, 0, AnyPropertyType,
            &target, &format, &size, &itemAmount, (unsigned char **)&data);

        // We have to allocate for incremental transfers but we don't have to
        // allocate for non-incremental transfers. I'm allocating for both here
        // to make _UIClipboardReadTextEnd work the same for both
        if (target != ui.incrID) {
            *bytes = size;
            char *copy = (char *)UI_MALLOC(*bytes);
            memcpy(copy, data, *bytes);
            XFree(data);
            XDeleteProperty(ui.copyEvent.xselection.display,
                            ui.copyEvent.xselection.requestor,
                            ui.copyEvent.xselection.property);
            return copy;
        }
        ...
        *bytes = 0;
        ...
        while (true) {
            ...
            if (ui.copyEvent.type == PropertyNotify) {
                // The other case - PropertyDelete would be caused by us and can
                // be ignored
                if (ui.copyEvent.xproperty.state == PropertyNewValue) {
                    ...
                    XGetWindowProperty(
                        ui.display, ui.copyEvent.xproperty.window,
                        ui.copyEvent.xproperty.atom, 0L, ~0L, True,
                        AnyPropertyType, &target, &format, &chunkSize,
        ...
        // TODO What should happen in this case? Is the next event always going
        // to be the selection event?
        return NULL;
    }
}
...
void UIInitialise() {
    ...
    ui.cursors[UI_CURSOR_SPLIT_V] =
        XCreateFontCursor(ui.display, XC_sb_v_double_arrow);
    ui.cursors[UI_CURSOR_SPLIT_H] =
        XCreateFontCursor(ui.display, XC_sb_h_double_arrow);
    ui.cursors[UI_CURSOR_FLIPPED_ARROW] =
        XCreateFontCursor(ui.display, XC_right_ptr);
    ui.cursors[UI_CURSOR_CROSS_HAIR] =
        XCreateFontCursor(ui.display, XC_crosshair);
    ui.cursors[UI_CURSOR_HAND] = XCreateFontCursor(ui.display, XC_hand1);
    ui.cursors[UI_CURSOR_RESIZE_UP] =
        XCreateFontCursor(ui.display, XC_top_side);
    ui.cursors[UI_CURSOR_RESIZE_LEFT] =
        XCreateFontCursor(ui.display, XC_left_side);
    ui.cursors[UI_CURSOR_RESIZE_UP_RIGHT] =
        XCreateFontCursor(ui.display, XC_top_right_corner);
    ui.cursors[UI_CURSOR_RESIZE_UP_LEFT] =
        XCreateFontCursor(ui.display, XC_top_left_corner);
    ui.cursors[UI_CURSOR_RESIZE_DOWN] =
        XCreateFontCursor(ui.display, XC_bottom_side);
    ui.cursors[UI_CURSOR_RESIZE_RIGHT] =
        XCreateFontCursor(ui.display, XC_right_side);
    ui.cursors[UI_CURSOR_RESIZE_DOWN_LEFT] =
        XCreateFontCursor(ui.display, XC_bottom_left_corner);
    ui.cursors[UI_CURSOR_RESIZE_DOWN_RIGHT] =
        XCreateFontCursor(ui.display, XC_bottom_right_corner);
...
void _UIWindowEndPaint(UIWindow *window, UIPainter *painter) {
    ...
    XPutImage(ui.display, window->window, DefaultGC(ui.display, 0),
              window->image, UI_RECT_TOP_LEFT(window->updateRegion),
              UI_RECT_TOP_LEFT(window->updateRegion),
              UI_RECT_SIZE(window->updateRegion));
}

void _UIWindowGetScreenPosition(UIWindow *window, int *_x, int *_y) {
    Window child;
    XTranslateCoordinates(ui.display, window->window,
                          DefaultRootWindow(ui.display), 0, 0, _x, _y, &child);
}
...
void UIMenuShow(UIMenu *menu) {
    ...
    for (int i = 0; i < ScreenCount(ui.display); i++) {
        ...
        int x, y;
        Window child;
        XTranslateCoordinates(ui.display, screen->root,
                              DefaultRootWindow(ui.display), 0, 0, &x, &y,
                              &child);

        if (menu->pointX >= x && menu->pointX < x + screen->width &&
            menu->pointY >= y && menu->pointY < y + screen->height) {
            if (menu->pointX + width > x + screen->width)
                menu->pointX = x + screen->width - width;
            if (menu->pointY + height > y + screen->height)
                menu->pointY = y + screen->height - height;
            if (menu->pointX < x)
                menu->pointX = x;
            if (menu->pointY < y)
                menu->pointY = y;
            if (menu->pointX + width > x + screen->width)
                width = x + screen->width - menu->pointX;
            if (menu->pointY + height > y + screen->height)
                height = y + screen->height - menu->pointY;
            break;
        }
    }
    ...
    XChangeProperty(ui.display, menu->e.window->window, properties[0], XA_ATOM,
                    32, PropModeReplace, (uint8_t *)properties, 2);
    XSetTransientForHint(ui.display, menu->e.window->window,
                         DefaultRootWindow(ui.display));
    ...
    hints.flags = 2;
    XChangeProperty(ui.display, menu->e.window->window, properties[2],
                    properties[2], 32, PropModeReplace, (uint8_t *)&hints, 5);

    XMapWindow(ui.display, menu->e.window->window);
    XMoveResizeWindow(ui.display, menu->e.window->window, menu->pointX,
                      menu->pointY, width, height);
}

void UIWindowPack(UIWindow *window, int _width) {
    int width = _width ? _width
                       : UIElementMessage(window->e.children[0],
                                          UI_MSG_GET_WIDTH, 0, 0);
    int height =
        UIElementMessage(window->e.children[0], UI_MSG_GET_HEIGHT, width, 0);
...
bool _UIProcessEvent(XEvent *event) {
    if (event->type == ClientMessage &&
        ...
        if (window->width != event->xconfigure.width ||
            ...
            window->height = event->xconfigure.height;
            window->bits = (uint32_t *)UI_REALLOC(
                window->bits, window->width * window->height * 4);
        ...
        if (event->xbutton.button >= 1 && event->xbutton.button <= 3) {
            _UIWindowInputEvent(
                window,
                (UIMessage)((event->type == ButtonPress ? UI_MSG_LEFT_DOWN
                                                        : UI_MSG_LEFT_UP) +
        ...
        if (event->xkey.x == 0x7123 && event->xkey.y == 0x7456) {
            ...
            // printf("%ld, %s\n", symbol, text);
            UIKeyTyped m = {0};
            m.textBytes = Xutf8LookupString(window->xic, &event->xkey, text,
        ...
        UIElementMessage(&window->e, UI_MSG_WINDOW_ACTIVATE, 0, 0);
               ...
               event->xclient.message_type == ui.dndEnterID) {
        UIWindow *window = _UIFindWindow(event->xclient.window);
        if (!window)
            return false;
        window->dragSource = (Window)event->xclient.data.l[0];
    } else if (event->type == ClientMessage &&
               event->xclient.message_type == ui.dndPositionID) {
        UIWindow *window = _UIFindWindow(event->xclient.window);
        if (!window)
            return false;
        ...
        XFlush(ui.display);
               ...
               event->xclient.message_type == ui.dndDropID) {
        UIWindow *window = _UIFindWindow(event->xclient.window);
        if (!window)
            return false;
        ...
        if (!XConvertSelection(ui.display, ui.dndSelectionID, ui.uriListID,
                               ui.primaryID, window->window,
        ...
        uint8_t *data = NULL;
        XGetWindowProperty(ui.display, window->window, ui.primaryID, 0, 65536,
        ...
        if (format == 8 /* bits per character */) {
            if (event->xselection.target == ui.uriListID) {
                ...
                UIElementMessage(&window->e, UI_MSG_WINDOW_DROP_FILES,
                                 fileCount, files);
        ...
        if ((XGetSelectionOwner(ui.display, ui.clipboardID) ==
             window->window) &&
            ...
            if (requestEvent.target == XA_STRING ||
                requestEvent.target == ui.textID ||
                requestEvent.target == utf8ID) {
                changePropertyResult = XChangeProperty(
                    requestEvent.display, requestEvent.requestor,
                    requestEvent.property, type, 8, PropModeReplace,
                    (const unsigned char *)ui.pasteText, strlen(ui.pasteText));
            } else if (requestEvent.target == ui.targetID) {
                changePropertyResult = XChangeProperty(
                    requestEvent.display, requestEvent.requestor,
                    requestEvent.property, XA_ATOM, 32, PropModeReplace,
            ...
            if (changePropertyResult == 0 || changePropertyResult == 1) {
                ...
                XSendEvent(ui.display, requestEvent.requestor, 0, 0,
                           (XEvent *)&sendEvent);
            }
        }
    }
...
void UIWindowPostMessage(UIWindow *window, UIMessage message, void *_dp) {
    ...
    event.state = message;
    event.type = KeyPress;
    XSendEvent(ui.display, window->window, True, KeyPressMask,
               (XEvent *)&event);
    XFlush(ui.display);
}
...
LRESULT CALLBACK _UIWindowProcedure(HWND hwnd, UINT message, WPARAM wParam,
                                    LPARAM lParam) {
    ...
    if (message == WM_CLOSE) {
        ...
        StretchDIBits(dc, 0, 0, UI_RECT_SIZE(window->e.bounds), 0, 0,
                      UI_RECT_SIZE(window->e.bounds), window->bits,
...
void UIMenuShow(UIMenu *menu) {
    int width, height;
    _UIMenuPrepare(menu, &width, &height);
    MoveWindow(menu->e.window->hwnd, menu->pointX, menu->pointY, width, height,
               FALSE);
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle,
    ...
    if (flags & UI_WINDOW_MENU) {
        ...
        window->hwnd =
            CreateWindowEx(WS_EX_TOPMOST | WS_EX_NOACTIVATE, "shadow", 0,
                           WS_POPUP, 0, 0, 0, 0, owner->hwnd, NULL, NULL, NULL);
    } else {
        window->hwnd = CreateWindowEx(
            WS_EX_ACCEPTFILES, "normal", cTitle, WS_OVERLAPPEDWINDOW,
            CW_USEDEFAULT, CW_USEDEFAULT, width ? width : CW_USEDEFAULT,
            height ? height : CW_USEDEFAULT, owner ? owner->hwnd : NULL, NULL,
            NULL, NULL);
    }
...
void _UIWindowEndPaint(UIWindow *window, UIPainter *painter) {
    ...
    info.biWidth = window->width, info.biHeight = window->height;
    info.biPlanes = 1, info.biBitCount = 32;
    StretchDIBits(dc, UI_RECT_TOP_LEFT(window->updateRegion),
                  UI_RECT_SIZE(window->updateRegion), window->updateRegion.l,
                  window->updateRegion.b + 1,
                  UI_RECT_WIDTH(window->updateRegion),
                  -UI_RECT_HEIGHT(window->updateRegion), window->bits,
                  (BITMAPINFO *)&info, DIB_RGB_COLORS, SRCCOPY);
...
void _UIMenuItemCallback(EsMenu *menu, EsGeneric context) {
    ((void (*)(void *))ui.menuData[context.u * 2 + 0])(
        ui.menuData[context.u * 2 + 1]);
}
...
void UIMenuAddItem(UIMenu *menu, uint32_t flags, const char *label,
    ...
    EsMenuAddItem(menu,
                  (flags & UI_BUTTON_CHECKED) ? ES_MENU_ITEM_CHECKED
                                              : ES_FLAGS_DEFAULT,
                  label, labelBytes, _UIMenuItemCallback, ui.menuIndex);
    ui.menuIndex++;
}

void UIMenuShow(UIMenu *menu) { EsMenuShow(menu); }
...
int _UIWindowCanvasMessage(EsElement *element, EsMessage *message) {
    ...
    if (!window) {
        ...
        EsRectangle bounds =
            ES_RECT_4PD(message->painter->offsetX, message->painter->offsetY,
                        window->width, window->height);
        EsDrawBitmap(message->painter, bounds, window->bits, window->width * 4,
                     0xFFFF);
        ...
        _UIWindowInputEvent(window, UI_MSG_MOUSE_WHEEL,
                            -message->scrollWheel.dy, 0);
    } else if (message->type == ES_MSG_MOUSE_MOVED ||
               message->type == ES_MSG_HOVERED_END ||
        ...
        return _UIWindowInputEvent(window, UI_MSG_KEY_TYPED, 0, &m) ? ES_HANDLED
                                                                    : 0;
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle,
    ...
    if (flags & UI_WINDOW_MENU) {
        ...
        window->canvas = EsCustomElementCreate(
            window->window, ES_CELL_FILL | ES_ELEMENT_FOCUSABLE);
